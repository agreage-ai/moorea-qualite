...
function etiquetteHTML(p: Record<string, string>): string {
  return `<div class="etiquette">
    <div class="rc"><b class="big rtl">اسم المنتج: ${p.nomArabe || ""}</b></div>
    <div class="rc"><b class="med">Product Name: ${p.nomAnglais || ""}</b></div>
    <hr/>
    <div class="r2"><b>Origin: ${p.origine || ""}</b><b class="rtl">المنشأ: ${p.origineArabe || ""}</b></div>
    <div class="r2"><b>Lot No: ${p.lotNo || ""}</b><b class="rtl">رقم اللوت:</b></div>
    <hr/>
    <div class="rc rtl"><b>${p.ingredientsArabe || ""}</b></div>
    <div class="rc"><b>Ingredients: ${p.ingredientsAnglais || ""}</b></div>
    <hr/>
    <div class="rc rtl"><b>نسبة الدسم: ${p.tauxMatiere || ""}٪ | نسبة الرطوبة: ${p.tauxHumidite || ""}٪</b></div>
    <hr/>
    <div class="r2"><b>Net Weight: ${p.poids || ""} KG</b><b class="rtl">الوزن الصافي: كجم</b></div>
    <hr/>
    <div class="rc"><b>ملاحظة Notice/</b></div>
    <div class="rc"><b class="sm">- Dairy was produced from an animal that did not show symptoms of anthrax during milking</b></div>
    <div class="rc"><b class="sm">- The milk was immediately cooled and heat treated at least enough for the pasteurization</b></div>
    <hr/>
    <div class="r2"><b>Prod. Date: ${p.prodDate || ""}</b><b class="rtl">تاريخ الإنتاج:</b></div>
    <div class="r2"><b>Exp. Date: ${p.expDate || ""}</b><b class="rtl">تاريخ الانتهاء:</b></div>
    <hr/>
    <div class="rc rtl"><b>المعلومات الغذائية لكل 100 غرام</b></div>
    <div class="rc rtl"><b class="sm">${p.nutritionArabe || ""}</b></div>
    <div class="rc"><b>Nutritional Value per 100 Grs</b></div>
    <div class="rc"><b class="sm">${p.nutritionAnglais || ""}</b></div>
    <hr/>
    <div class="r2"><b class="sm">Exporter: ${p.exporteur || ""}</b><b class="sm rtl">المصدر: ${p.exporteurArabe || ""}</b></div>
    <div class="r2"><b class="sm">Importer: ${p.importeur || ""}</b><b class="sm rtl">المستورد: ${p.importeurArabe || ""}</b></div>
    <div class="rc"><b class="sm">${p.website || ""}</b></div>
  </div>`;
}

function buildPrintHTML(produits: Record<string, string>[]): string {
  const css = `*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif}
.etiquette{width:100%;border:1px solid #000;padding:6mm;page-break-after:always;margin-bottom:8px}
.etiquette:last-child{page-break-after:avoid;margin-bottom:0}
hr{border:none;border-top:1px solid #000;margin:3px 0}
b{display:block;font-weight:bold;font-size:11pt;line-height:1.4}
b.big{font-size:14pt}b.med{font-size:12pt}b.sm{font-size:9pt}
b.rtl{direction:rtl;text-align:right;unicode-bidi:embed}
.rc{text-align:center;margin:2px 0}.rc b{display:inline}
.rtl{direction:rtl}
.r2{display:flex;justify-content:space-between;align-items:center;margin:2px 0}
.r2 b{flex:1}.r2 b.rtl{text-align:right}
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const extract = (pattern: RegExp) => { for (const l of lines) { const m = l.match(pattern); if (m) return m[1]?.trim() || ""; } return ""; };
        resolve({
          nomAnglais: extract(/Product Name:\s*(.+)/i),
          nomArabe: lines.find((l: string) => /[\u0600-\u06FF]{3,}/.test(l)) || "",
          origine: extract(/Origin:\s*([A-Za-z ]+)/i),
          origineArabe: extract(/المنشأ:\s*(.+)/),
          lotNo: extract(/Lot No[:\s]+([^\s]+)/i),
          poids: extract(/Net Weight[:\s]+([\d.,]+)/i),
          ingredientsArabe: lines.find((l: string) => /المكونات/.test(l)) || "",
...
export default function EtiquettesModule({ onClose }: { onClose: () => void }) {
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try { const p = await parseDocx(files[i]); await addDoc(collection(db, "etiquettes_produits"), p); ok++; }
      catch {}
    }
    await fetchProduits();
    setImporting(false); setImportProgress(null); e.target.value = "";
    if (files.length === 1) {
      const snap = await getDocs(collection(db, "etiquettes_produits"));
  ...
  async function supprimer(id: string) {
    ...
    await deleteDoc(doc(db, "etiquettes_produits", id)); await fetchProduits();
  }
  ...
  function ouvrirLot() {
    ...
    setLotSelects(sel); setLotVars(vars); setVue("lot");
  }

  function genererLot() {
    const sel = produits.filter(p => lotSelects[p.id]);
    if (!sel.length) { alert("Sélectionne au moins une étiquette"); return; }
    setPrintHTML(buildPrintHTML(sel.map(p => ({ ...p, ...lotVars[p.id] }))));
    sel.forEach(p => { if (lotVars[p.id]) updateDoc(doc(db, "etiquettes_produits", p.id), lotVars[p.id]).catch(() => {}); });
  }

  const nbSel = Object.values(lotSelects).filter(Boolean).length;
    ...
    <div style={{ minHeight: "100vh", background: "#f5f3ee", fontFamily: "'Syne', sans-serif" }}>
      {topBar(`🖨️ Lot — ${nbSel} sélectionnée${nbSel > 1 ? "s" : ""}`, <>
        <button style={{ ...btn("#e8e0d0", "#374151"), fontSize: 12 }}
          onClick={() => { const all = nbSel === produits.length; const s: Record<string, boolean> = {}; produits.forEach(p => { s[p.id] = !all; }); setLotSelects(s); }}>
          {nbSel === produits.length ? "Tout désélectionner" : "Tout sélectionner"}
        </button>
        <button style={btn("#16a34a")} onClick={genererLot} disabled={nbSel === 0}>
          🖨️ Imprimer {nbSel}
        </button>
      </>)}
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "16px 12px 80px" }}>
